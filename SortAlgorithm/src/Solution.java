...
public class Solution {
    ...
    private static ArrayList<Integer> merge(ArrayList<Integer> nums, int left, int mid, int right){
        ...
        ArrayList<Integer> temp = new ArrayList<>(len); //指定长度,辅助空间
        int i = left;   //前一子数组的起始元素
        int j = mid + 1;    //子数组的起始元素
        while(i<=mid && j<=right){
    ...
    public static ArrayList<Integer> mergeSortIteration(ArrayList<Integer> nums, int len){  //迭代，自底向上
        if(nums.size()<=1){
            return nums;
        }
        int left;
    ...
    private static int RandomInRange(int start,int end){
        if(start<end){
            ...
            return random.nextInt(end-start+1)+start;   //return random.nextInt(10)+5;获取[5,10)范围内的随机整数
        }
        return start;
    }
    ...
    /*
    * （1）按个位排序，个位小的数在前面，即49不会在44,45的前面
    * （2）所以再次按十位排时，虽然十位相同，但也只会44,45被先扫描入库，49被后扫描入库
    * */
}
